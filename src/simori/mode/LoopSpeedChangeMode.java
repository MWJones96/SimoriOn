 ...
 * is pressed. here all the buttons are cleared as usual, when a button is
 * clicked, the specific row and column will be selected. within this class, the
 * range of speed that can be chosen can be within the boundaries of 0-160bpm
 * 
 * @author team G
 * 
 */
public class LoopSpeedChangeMode implements Mode {
	...
	/**
	 * constructor used to set the default state for the loop speed change mode
	 */
	...
	/**
	 * shows that the matrix button has been pressed in the correct mode so that
	 * way the loop speed can be changed. if the speed is 0, then the instrument
	 * being played will be silent. once the oK button is pressed, then it will
	 * go back to performance mode.
	 * 
	 * @param button
	 */
	@Override
	public void processMatrixButton(GridButton button) {
		...
		SimoriOn.getInstance().getGui()
				.writeToLCD("Loop Speed: " + Integer.toString(this.speed));
